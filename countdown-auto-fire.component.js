(function (runtime) {
  ...
  runtime.register({
    ...
    signals: [
      ...
      { key: "label", name: "Standard name", type: "serial", direction: "input", defaultValue: "CountdownAutoFire.Label" },
      { key: "armedLabel", name: "Armed name", type: "serial", direction: "input", defaultValue: "CountdownAutoFire.ArmedLabel" },
      { key: "completedLabel", name: "Completed name", type: "serial", direction: "input", defaultValue: "CountdownAutoFire.CompletedLabel" },
    ...
    mount(root, context) {
      ...
      const configuredDuration = Math.max(0, Math.min(60, Number(p.countdownSeconds) || 0)) * 1000;
      ...
      theme.textContent = '[data-component="countdown-auto-fire"] .countdown-button.pressed{transform:scale(.97);filter:brightness(1.16)}[data-component="countdown-auto-fire"] .countdown-button.armed{color:var(--selected-text-color);border-color:var(--selected-border-color);background:linear-gradient(145deg,rgba(255,255,255,.16),rgba(0,0,0,.16)),var(--selected-color);box-shadow:inset 0 1px rgba(255,255,255,.44),0 0 var(--glow-strength-px) var(--selected-glow-color)}[data-component="countdown-auto-fire"] .countdown-button.completed{color:var(--completed-text-color);border-color:var(--completed-border-color);background:linear-gradient(145deg,rgba(255,255,255,.22),rgba(0,0,0,.1)),var(--completed-color);box-shadow:inset 0 1px rgba(255,255,255,.48),0 0 var(--glow-strength-px) var(--completed-glow-color);animation:countdown-fire .45s cubic-bezier(.2,.9,.3,1.2)}@keyframes countdown-fire{0%{transform:scale(.88);filter:brightness(1.7)}60%{transform:scale(1.07)}100%{transform:scale(1);filter:brightness(1)}}';
      ...
      const unbindPointer = context.interactions.bindPrimaryPointer(button, { down: () => button.classList.add("pressed"), up: () => { button.classList.remove("pressed"); if (completed) reset(); else if (armed) cancel(true); else arm(true); }, cancel: () => button.classList.remove("pressed") });
      ...
      context.signals.subscribe("countdownTime", value => { const n = Math.max(0, Number(value) || 0); duration = n > 0 ? Math.min(60, n > 60 ? n / 65535 * 60 : n) * 1000 : configuredDuration; });
      render();
      return () => { cancelAnimationFrame(frame); clearTimeout(resetTimer); unbindPointer(); };
    },
  });
