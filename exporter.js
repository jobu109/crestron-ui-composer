(function (global) {
  ...
  function exportProject(project) {
    ...
    const usedIconValues = [
            ...
            Object.values(item.properties || {}).flatMap((value) =>
              typeof value === "string"
                ? value
                    .split("|")
                    .filter((entry) =>
                      /^(?:fa-(?:solid|regular)|mdi|bi):[a-z0-9-]+$/i.test(entry),
                    )
                : [],
            ),
          ),
      ...
      iconDefinitions =
        global.ComposerIcons?.pick(usedIconValues) || {};
    ...
    const holdController = feedbackController.replace(
      ...
      animatedController = layeredController
        .replace(
          ...
          "signals:signals,icons:window.ComposerIcons,interactions:{bindPrimaryPointer:bindPrimaryPointer},resolveComponent:function(id){return definitions[id]},decorateButtonLabel:wireButtonLabel,decorateIconPlacement:wireIconPlacement,navigate:show,options:",
        )
        .replace(
        ...
        .replace(
          ...
          "subscribe:function(key,callback){var spec=def.signals.find(function(s){return s.key===key}),binding=item.bindings[key],handler=key==='selected'?function(value){holder.dataset.assetSelected=value===true||value===1||value==='1'?'true':'false';callback(value)}:callback;if(!spec||!binding||!binding.value||spec.optionalProperty&&!enabled(item.properties[spec.optionalProperty]))return;subscribeAddress(spec.type,binding.value,handler)}",
        )
    ...
    const contractController = animatedController
      ...
      iconRuntime = `/* Font Awesome Free ${global.ComposerIcons?.version || "7"} CC BY 4.0; Material Design Icons Apache 2.0; Bootstrap Icons MIT. */(function(){var definitions=${JSON.stringify(iconDefinitions)};function escapeAttribute(value){return String(value||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}function get(value){return definitions[String(value||'').replace(/^fas:/,'fa-solid:').replace(/^far:/,'fa-regular:')]||null}function paths(record){var value=record&&record[2];if(typeof value==='string'&&value.trim().charAt(0)==='<')return value;return(Array.isArray(value)?value:[value]).filter(Boolean).map(function(path,index){return'<path fill="currentColor" stroke="none"'+(index?' opacity=".4"':'')+' d="'+escapeAttribute(path)+'"></path>'}).join('')}function svg(value,options){options=options||{};var record=get(value),className=options.className?' class="'+escapeAttribute(options.className)+'"':'',label=options.label?' aria-label="'+escapeAttribute(options.label)+'" role="img"':' aria-hidden="true"',body,viewBox;if(record){viewBox='0 0 '+record[0]+' '+record[1];body=paths(record)}else{viewBox=options.viewBox||'0 0 24 24';body=options.legacy&&options.legacy[value]||options.legacy&&options.legacy[options.fallback]||options.body||''}return'<svg'+className+' viewBox="'+viewBox+'"'+label+' fill="currentColor" focusable="false">'+body+'</svg>'}window.ComposerIcons={get:get,svg:svg};})();`,
      safeController = (iconRuntime + restoredController).replace(
        /<\/script/gi,
        "<\\/script",
      );
