(function (runtime) {
  ...
  runtime.register({
    ...
    properties: [
      ...
      { key: "selectedText", name: "Selected text", type: "text", defaultValue: "" },
      ...
      { key: "selectedContentMode", name: "Selected face content", type: "select", options: [{ value: "", label: "Match standard" }, { value: "symbol", label: "Symbol" }, { value: "text", label: "Text" }, { value: "both", label: "Symbol and text" }], defaultValue: "" },
      ...
      { key: "selectedSymbol", name: "Selected symbol", type: "select", options: [{ value: "", label: "Match standard" }, { value: "⏻", label: "Power" }, { value: "⏼", label: "Standby" }, { value: "▶", label: "Play" }, { value: "■", label: "Stop" }, { value: "●", label: "Circle" }], defaultValue: "" },
    ...
    signals: [
      ...
      { key: "name", name: "Name", type: "serial", direction: "input", defaultValue: "IlluminatedPowerButton.Label" },
    ],
    ...
    mount(root, context) {
      ...
      let remoteName = "", currentOn = false;
      ring.style.boxShadow = "inset 0 4px 7px rgba(255,255,255,.2), inset 0 -7px 11px #000";
      function renderContent(){const mode=currentOn?(p.selectedContentMode||p.contentMode||"symbol"):(p.contentMode||"symbol"),chosenSymbol=currentOn?(p.selectedSymbol||p.symbol||"⏻"):(p.symbol||"⏻"),nextText=remoteName||(currentOn&&p.selectedText?p.selectedText:fallback);face.className="ipb-face mode-"+mode;if(chosenSymbol==="⏻")symbol.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2v9"></path><path d="M7.1 5.7a8 8 0 1 0 9.8 0"></path></svg>';else symbol.textContent=chosenSymbol;symbol.hidden=mode==="text";symbol.style.display=mode==="text"?"none":"";text.hidden=mode==="symbol";text.style.display=mode==="symbol"?"none":"";text.textContent=nextText;name.textContent=nextText}
      renderContent();
      ...
      context.signals.subscribe("name", (value) => { remoteName = value == null || value === "" ? "" : String(value); renderContent(); });
