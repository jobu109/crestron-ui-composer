(function (runtime) {
  ...
  runtime.register({
    ...
    styles: '[data-component="illuminated-power-button"]{display:block;width:100%;height:100%;padding:6%;box-sizing:border-box;font-family:"Segoe UI",sans-serif}[data-component="illuminated-power-button"] *{box-sizing:border-box}[data-component="illuminated-power-button"] .ipb-root{display:grid;grid-template-rows:minmax(0,1fr) auto;gap:7px;width:100%;height:100%;align-items:center;justify-items:center}[data-component="illuminated-power-button"] .ipb-button{width:min(100%,100vh);height:min(100%,100vw);max-width:100%;max-height:100%;aspect-ratio:1;padding:8%;border:0;border-radius:50%;appearance:none;background:radial-gradient(circle at 35% 28%,color-mix(in srgb,var(--rim-color) 65%,#fff),var(--rim-color) 58%,#030404);box-shadow:inset 0 4px 7px rgba(255,255,255,.16),inset 0 -9px 14px #000,0 9px 16px rgba(0,0,0,.55);cursor:pointer;touch-action:none}[data-component="illuminated-power-button"] .ipb-ring{display:flex;width:100%;height:100%;padding:8%;border-radius:50%;background:var(--off-color);box-shadow:0 0 var(--glow-strength-px) var(--off-color),inset 0 2px 5px rgba(255,255,255,.6),inset 0 -4px 7px rgba(0,0,0,.5);transition:background .22s,box-shadow .22s}[data-component="illuminated-power-button"] .ipb-face{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2%;width:100%;height:100%;overflow:hidden;border:2px solid rgba(255,255,255,.13);border-radius:50%;background:radial-gradient(circle at 40% 32%,color-mix(in srgb,var(--face-color) 82%,#fff),var(--face-color) 62%,color-mix(in srgb,var(--face-color) 58%,#000));box-shadow:inset 0 7px 11px rgba(255,255,255,.1),inset 0 -10px 16px rgba(0,0,0,.65);color:var(--off-content-color);transition:color .22s,transform .12s}[data-component="illuminated-power-button"] .ipb-symbol{font:700 var(--symbol-size-px)/1 "Segoe UI Symbol","Arial Unicode MS",sans-serif;text-shadow:0 0 calc(var(--glow-strength-px) * .75) currentColor}[data-component="illuminated-power-button"] .ipb-text{max-width:82%;overflow:hidden;font:800 var(--text-size-px)/1.1 "Segoe UI",sans-serif;text-overflow:ellipsis;text-shadow:0 0 calc(var(--glow-strength-px) * .55) currentColor;white-space:nowrap}[data-component="illuminated-power-button"] .ipb-button.on .ipb-ring{background:var(--on-color);box-shadow:0 0 var(--glow-strength-px) var(--on-color),inset 0 2px 5px rgba(255,255,255,.65),inset 0 -4px 7px rgba(0,0,0,.45)}[data-component="illuminated-power-button"] .ipb-button.on .ipb-face{color:var(--on-content-color)}[data-component="illuminated-power-button"] .ipb-button.pressed .ipb-face{transform:scale(.94);filter:brightness(1.2)}[data-component="illuminated-power-button"] .ipb-name{max-width:100%;overflow:hidden;color:var(--name-color);font:800 var(--text-size-px)/1.1 "Segoe UI",sans-serif;text-align:center;text-overflow:ellipsis;text-shadow:0 2px 4px #000;white-space:nowrap}',
    mount(root, context) {
      const button = root.querySelector(".ipb-button"), symbol = root.querySelector(".ipb-symbol"), text = root.querySelector(".ipb-text"), name = root.querySelector(".ipb-name"), p = context.options.properties || {}, fallback = p.text || "POWER";
