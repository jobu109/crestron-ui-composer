(function (runtime) {
  ...
  runtime.register({
    ...
    properties: [
      ...
      { key: "selectedTitle", name: "Selected title", type: "text", defaultValue: "" },
      { key: "localArtist", name: "Default artist", type: "text", defaultValue: "Steven Blake" },
      { key: "selectedArtist", name: "Selected artist", type: "text", defaultValue: "" },
      ...
      { key: "selectedIcon", name: "Selected icon", type: "select", options: [
        { value: "", label: "Match standard" }, { value: "music", label: "Music notes" }, { value: "album", label: "Album" },
        { value: "play", label: "Play" }, { value: "radio", label: "Radio" },
        { value: "headphones", label: "Headphones" }, { value: "none", label: "None" }
      ], defaultValue: "" },
      ...
      { key: "selectedAccentColor", name: "Selected accent / wave color", type: "color", defaultValue: "#ff4d6d" },
      { key: "titleColor", name: "Title color", type: "color", defaultValue: "#f2f4f7" },
      { key: "selectedTitleColor", name: "Selected title color", type: "color", defaultValue: "#f2f4f7" },
      { key: "artistColor", name: "Artist color", type: "color", defaultValue: "#ff4d6d" },
      { key: "selectedArtistColor", name: "Selected artist color", type: "color", defaultValue: "#ff4d6d" },
    ...
    signals: [
      ...
      { key: "title", name: "Name", type: "serial", direction: "input", defaultValue: "MusicCard.Label" },
      { key: "artist", name: "Artist Name", type: "serial", direction: "input", defaultValue: "MusicCard.ArtistLabel" }
    ],
    ...
    mount(root, context) {
      const p = context.options.properties || {}, card = root.querySelector(".music-card"), icon = root.querySelector(".music-icon"), iconWrap = root.querySelector(".music-icon-wrap"), title = root.querySelector(".music-title"), artist = root.querySelector(".music-artist"), wave = root.querySelector(".music-wave"), truthy = (value, fallback) => value == null ? fallback : value === true || value === 1 || value === "1" || String(value).toLowerCase() === "true", paths = { music: '<path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>', album: '<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3"/><path d="M12 3v6"/>', play: '<path d="M8 5l11 7-11 7z"/>', radio: '<rect x="3" y="7" width="18" height="13" rx="2"/><path d="M7 7l10-4M7 12h4"/><circle cx="16" cy="14" r="3"/>', headphones: '<path d="M4 14v-2a8 8 0 0 1 16 0v2M4 14h3v6H5a1 1 0 0 1-1-1v-5zm16 0h-3v6h2a1 1 0 0 0 1-1v-5z"/>', none: "" }; let playing = truthy(p.animateWhileIdle, true), level = 100, timer = 0, selected = false, remoteTitle = "", remoteArtist = "";
      ...
      function renderState(){const iconKey=selected?(p.selectedIcon||p.icon||"music"):(p.icon||"music");card.style.background=selected?(p.selectedSurfaceColor||p.surfaceColor||"#2c3038"):(p.surfaceColor||"#2c3038");root.style.setProperty("--accent",selected?(p.selectedAccentColor||p.accentColor||"#ff4d6d"):(p.accentColor||"#ff4d6d"));root.style.setProperty("--title",selected?(p.selectedTitleColor||p.titleColor||"#f2f4f7"):(p.titleColor||"#f2f4f7"));root.style.setProperty("--artist",selected?(p.selectedArtistColor||p.artistColor||p.accentColor||"#ff4d6d"):(p.artistColor||p.accentColor||"#ff4d6d"));title.textContent=remoteTitle||(selected&&p.selectedTitle?p.selectedTitle:String(p.localTitle??"ROSE MUSIC"));artist.textContent=remoteArtist||(selected&&p.selectedArtist?p.selectedArtist:String(p.localArtist??"Steven Blake"));iconWrap.innerHTML=context.icons.svg(iconKey,{className:"music-icon",legacy:paths});iconWrap.style.display=truthy(p.showIcon,true)&&iconKey!=="none"?"":"none"}
      renderState(); title.style.display = truthy(p.showTitle, true) ? "" : "none"; artist.style.display = truthy(p.showArtist, true) ? "" : "none"; wave.style.display = truthy(p.showWave, true) ? "" : "none";
      ...
      context.signals.subscribe("selected", value => { selected = truthy(value, false); iconWrap.classList.toggle("active", selected); if (!selected) iconWrap.classList.remove("pressed"); renderState(); }); context.signals.subscribe("playing", value => { playing = truthy(value, false); renderWave(); }); context.signals.subscribe("level", value => { const input = Number(value) || 0; level = Math.max(0, Math.min(100, input > 100 ? input / 65535 * 100 : input)); renderWave(); }); context.signals.subscribe("title", value => { remoteTitle=value==null?"":String(value);renderState(); }); context.signals.subscribe("artist", value => { remoteArtist=value==null?"":String(value);renderState(); }); restart(); return () => { clearInterval(timer); iconWrap.removeEventListener("pointerdown", down); iconWrap.removeEventListener("pointerup", up); iconWrap.removeEventListener("pointerleave", up); iconWrap.removeEventListener("pointercancel", up); };
    }
  });
