(function (runtime) {
  ...
  runtime.register({
    ...
    styles:
      ...
      '[data-component="rotary-knob"] .rotary-knob{--gauge-progress:0%;--gauge-color:var(--low-color);position:relative;height:100%;max-height:100%;aspect-ratio:1;border:5px solid var(--ring-color);border-radius:50%;background:conic-gradient(from 225deg,var(--gauge-color) 0 var(--gauge-progress),#111 var(--gauge-progress) 75%,#111 75% 100%);box-shadow:inset 0 0 0 2px rgba(0,0,0,.88),0 0 var(--glow-strength-px) color-mix(in srgb,var(--glow-color) 70%,transparent);cursor:pointer;outline:none;touch-action:none}' +
