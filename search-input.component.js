(function (runtime) {
  ...
  runtime.register({
    ...
    mount(root, context) {
      const input = root.querySelector(".search-native"), field = root.querySelector(".search-field"), button = root.querySelector(".search-submit"), icon = root.querySelector(".search-svg"), shell = root.querySelector(".search-root"), p = context.options.properties || {}, sendAsTyped = p.sendAsTyped === true || String(p.sendAsTyped).toLowerCase() === "true";
      ...
      function press(event) { context.signals.publish("searchPress", true); if (!shell.classList.contains("open")) shell.classList.add("open"); else send(); input.focus(); event.preventDefault(); event.stopPropagation(); }
      ...
      function outside(event) { if (!root.contains(event.target)) shell.classList.remove("open"); }
