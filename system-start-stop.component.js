(function (runtime) {
  ...
  runtime.register({
    ...
    template: '<div class="sss-root"><button class="sss-button" type="button" aria-label="System start stop"><canvas width="300" height="300"></canvas><span class="sss-face"><span class="sss-vortex"></span><span class="sss-pill"></span><span class="sss-system">SYSTEM</span><span class="sss-label sss-start">START</span><span class="sss-label sss-stop">STOP</span></span></button></div>',
    ...
    mount(root, context) {
      const button = root.querySelector(".sss-button"), canvas = root.querySelector("canvas"), ctx = canvas.getContext("2d"), face = root.querySelector(".sss-face"), vortex = root.querySelector(".sss-vortex"), pill = root.querySelector(".sss-pill"), system = root.querySelector(".sss-system"), startLabel = root.querySelector(".sss-start"), stopLabel = root.querySelector(".sss-stop"), p = context.options.properties || {};
      ...
      vortex.style.cssText = "position:absolute;z-index:0;inset:-14%;border-radius:50%;opacity:0;background:repeating-conic-gradient(from 0deg,rgba(255,255,255,.86) 0deg,rgba(82,91,97,.74) 8deg,rgba(6,8,10,.96) 19deg,rgba(196,209,218,.74) 29deg,rgba(18,21,24,.96) 43deg);box-shadow:inset 0 0 28px #000,0 0 22px rgba(210,225,235,.45);filter:contrast(1.4) brightness(.88);-webkit-mask-image:radial-gradient(circle,#000 0 54%,rgba(0,0,0,.95) 66%,transparent 79%);mask-image:radial-gradient(circle,#000 0 54%,rgba(0,0,0,.95) 66%,transparent 79%);transform:scale(1.15) rotate(0deg);transform-origin:center;will-change:transform,opacity";
      ...
      function stable(value){running=targetRunning=!!value;button.classList.toggle("on",running);button.setAttribute("aria-pressed",running?"true":"false");vortex.style.opacity="0";vortex.style.transform="rotate(0deg)";ring(running?on:off,running?onGlow:offGlow,0,-Math.PI/2);}
      function animate(next){next=!!next;if(frame)cancelAnimationFrame(frame);targetRunning=next;const from=running?on:off,to=next?on:off,fromGlow=running?onGlow:offGlow,toGlow=next?onGlow:offGlow,duration=Math.max(300,Number(next?p.startDuration:p.stopDuration)||2000),spins=next?3.6:3,t0=performance.now();function step(now){const raw=Math.min(1,(now-t0)/duration),e=ease(raw),angle=-Math.PI/2+Math.PI*2*spins*e,vortexPhase=raw<.5?ease(raw*2):ease((1-raw)*2),hole=68*(1-vortexPhase),edge=Math.min(76,hole+7),closed=vortexPhase>.92,closure=Math.max(0,(vortexPhase-.7)/.3),mask=closed?"radial-gradient(circle,#000 0 77%,transparent 83%)":"radial-gradient(circle,transparent 0 "+hole+"%,rgba(0,0,0,.96) "+edge+"%,#000 77%,transparent 83%)";ring(mix(from,to,e),mix(fromGlow,toGlow,e),raw,angle);vortex.style.opacity=String(Math.min(1,vortexPhase*1.7)*vortexIntensity);vortex.style.transform="rotate("+(spins*720*e)+"deg)";vortex.style.webkitMaskImage=mask;vortex.style.maskImage=mask;vortex.style.background="radial-gradient(circle,rgba(184,194,200,"+(closure*.8)+") 0%,rgba(68,75,80,"+(closure*.74)+") 48%,rgba(10,13,15,"+(closure*.45)+") 72%,transparent 79%),repeating-conic-gradient(from 0deg,rgba(255,255,255,.86) 0deg,rgba(82,91,97,.74) 8deg,rgba(6,8,10,.96) 19deg,rgba(196,209,218,.74) 29deg,rgba(18,21,24,.96) 43deg)";vortex.style.filter="contrast("+(1.4-closure*.35)+") brightness("+(.72+vortexPhase*.5)+") blur("+(closed?1.1:Math.max(0,(1-vortexPhase)*.7))+"px)";if(raw>=.5)button.classList.toggle("on",next);if(raw<1)frame=requestAnimationFrame(step);else{frame=0;stable(next);}}frame=requestAnimationFrame(step);}
