(function (runtime) {
  ...
  runtime.register({
    ...
    mount(root, context) {
      ...
      function animate(next){next=!!next;if(frame)cancelAnimationFrame(frame);targetRunning=next;const from=running?on:off,to=next?on:off,fromGlow=running?onGlow:offGlow,toGlow=next?onGlow:offGlow,duration=Math.max(300,Number(next?p.startDuration:p.stopDuration)||2000),spins=next?3.6:3,t0=performance.now();function step(now){const raw=Math.min(1,(now-t0)/duration),e=ease(raw),angle=-Math.PI/2+Math.PI*2*spins*e,vortexPhase=raw<.5?ease(raw*2):ease((1-raw)*2),hole=68*(1-vortexPhase),edge=Math.min(76,hole+7),mask="radial-gradient(circle,transparent 0 "+hole+"%,rgba(0,0,0,.96) "+edge+"%,#000 77%,transparent 83%)";ring(mix(from,to,e),mix(fromGlow,toGlow,e),raw,angle);vortex.style.opacity=String(Math.min(1,vortexPhase*1.7)*vortexIntensity);vortex.style.transform="rotate("+(spins*720*e)+"deg)";vortex.style.webkitMaskImage=mask;vortex.style.maskImage=mask;vortex.style.filter="contrast(1.4) brightness("+(.72+vortexPhase*.5)+") blur("+Math.max(0,(1-vortexPhase)*.7)+"px)";if(raw>=.5)button.classList.toggle("on",next);if(raw<1)frame=requestAnimationFrame(step);else{frame=0;stable(next);}}frame=requestAnimationFrame(step);}
