(function (runtime) {
  ...
  runtime.register({
    ...
    properties: [
      ...
      { key: "startDuration", name: "Start animation (ms)", type: "number", min: 500, max: 12000, step: 100, defaultValue: 4200 },
      { key: "stopDuration", name: "Stop animation (ms)", type: "number", min: 500, max: 12000, step: 100, defaultValue: 4200 },
      { key: "defaultRunning", name: "Default running state", type: "checkbox", defaultValue: false },
    ],
    signals: [
      { key: "press", name: "Press", type: "digital", direction: "output", defaultValue: "SystemStartStop.Press" },
      { key: "startSelected", name: "Start Selected", type: "digital", direction: "input", defaultValue: "SystemStartStop.StartSelected" },
      { key: "stopSelected", name: "Stop Selected", type: "digital", direction: "input", defaultValue: "SystemStartStop.StopSelected" },
    ],
    template: '<div class="sss-root"><button class="sss-button" type="button" aria-label="System start stop"><canvas width="300" height="300"></canvas><span class="sss-face"><span class="sss-vortex"><span class="sss-panel"></span><span class="sss-panel"></span><span class="sss-panel"></span><span class="sss-panel"></span><span class="sss-panel"></span><span class="sss-panel"></span></span><span class="sss-pill"></span><span class="sss-system">SYSTEM</span><span class="sss-label sss-start">START</span><span class="sss-label sss-stop">STOP</span></span></button></div>',
    ...
    mount(root, context) {
      const button = root.querySelector(".sss-button"), canvas = root.querySelector("canvas"), ctx = canvas.getContext("2d"), face = root.querySelector(".sss-face"), vortex = root.querySelector(".sss-vortex"), panels = [...root.querySelectorAll(".sss-panel")], pill = root.querySelector(".sss-pill"), system = root.querySelector(".sss-system"), startLabel = root.querySelector(".sss-start"), stopLabel = root.querySelector(".sss-stop"), p = context.options.properties || {};
      ...
      function placePanels(phase){panels.forEach((panel,index)=>{const base=index*60,curve=(1-phase)*48+phase*22,travel=(1-phase)*82,scale=.28+phase*.72;panel.style.transform="rotate("+(base+curve)+"deg) translateX("+travel+"%) scale("+scale+")";});}
      function stable(value){running=targetRunning=!!value;button.classList.toggle("on",running);button.setAttribute("aria-pressed",running?"true":"false");vortex.style.opacity="0";vortex.style.transform="rotate(0deg)";placePanels(0);ring(running?on:off,running?onGlow:offGlow,0,-Math.PI/2);}
      function animate(next){next=!!next;if(frame)cancelAnimationFrame(frame);targetRunning=next;const from=running?on:off,to=next?on:off,fromGlow=running?onGlow:offGlow,toGlow=next?onGlow:offGlow,duration=Math.max(300,Number(next?p.startDuration:p.stopDuration)||2000),spins=next?3.6:3,t0=performance.now();function step(now){const raw=Math.min(1,(now-t0)/duration),e=ease(raw),angle=-Math.PI/2+Math.PI*2*spins*e,panelPhase=raw<.5?ease(raw*2):ease((1-raw)*2),turn=210*e+55*panelPhase;ring(mix(from,to,e),mix(fromGlow,toGlow,e),raw,angle);vortex.style.opacity=String(Math.min(1,panelPhase*1.8)*vortexIntensity);vortex.style.transform="rotate("+turn+"deg)";placePanels(panelPhase);if(raw>=.5)button.classList.toggle("on",next);if(raw<1)frame=requestAnimationFrame(step);else{frame=0;stable(next);}}frame=requestAnimationFrame(step);}
      function release(){button.classList.remove("pressed");if(pressedKey)context.signals.publish(pressedKey,false);pressedKey="";}
      function press(event){release();pressedKey="press";button.classList.add("pressed");context.signals.publish(pressedKey,true);event.preventDefault();}
      ...
      context.signals.subscribe("startSelected",value=>{const selected=value===true||value===1||value==="1";if(selected&&!targetRunning)animate(true);});
      context.signals.subscribe("stopSelected",value=>{const selected=value===true||value===1||value==="1";if(selected&&targetRunning)animate(false);});
      stable(running);
