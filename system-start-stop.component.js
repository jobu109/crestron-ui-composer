(function (runtime) {
  ...
  runtime.register({
    ...
    mount(root, context) {
      ...
      panels.forEach((panel,index)=>{panel.style.cssText="position:absolute;left:50%;top:25%;width:66%;height:50%;clip-path:polygon(0 50%,100% 0,100% 100%);background:linear-gradient("+(index%2?"145deg":"35deg")+",#0d1114 0%,#778188 22%,#252b30 48%,#b9c2c7 69%,#121619 100%);border-right:2px solid #080a0b;box-shadow:inset 7px 0 12px rgba(255,255,255,.2),inset -10px 0 17px rgba(0,0,0,.7),3px 0 7px #000;transform-origin:0 50%;will-change:transform";});
      ...
      function placePanels(phase){panels.forEach((panel,index)=>{const base=index*60,curve=(1-phase)*42,travel=(1-phase)*118,scale=.34+phase*.66;panel.style.transform="rotate("+(base+curve)+"deg) translateX("+travel+"%) scaleX("+scale+")";});}
