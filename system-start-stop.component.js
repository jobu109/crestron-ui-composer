(function (runtime) {
  ...
  runtime.register({
    ...
    mount(root, context) {
      ...
      panels.forEach((panel,index)=>{panel.style.cssText="position:absolute;left:50%;top:22%;width:82%;height:56%;clip-path:polygon(0 50%,100% 0,83% 100%);background:linear-gradient("+(index%2?"145deg":"35deg")+",#576269 0%,#aeb8bd 20%,#707c83 44%,#e1e7ea 68%,#657077 100%);border-right:2px solid #3f484d;box-shadow:inset 7px 0 12px rgba(255,255,255,.34),inset -10px 0 17px rgba(42,49,53,.44),3px 0 7px rgba(38,44,48,.72);transform-origin:0 50%;will-change:transform";});
      ...
      function placePanels(phase){panels.forEach((panel,index)=>{const base=index*60,curve=(1-phase)*55+phase*22,travel=(1-phase)*125,scale=.32+phase*.88;panel.style.transform="rotate("+(base+curve)+"deg) translateX("+travel+"%) scaleX("+scale+")";});}
