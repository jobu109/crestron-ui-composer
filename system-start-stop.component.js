(function (runtime) {
  ...
  runtime.register({
    ...
    mount(root, context) {
      ...
      function stable(value){running=targetRunning=!!value;button.classList.toggle("on",running);button.setAttribute("aria-pressed",running?"true":"false");vortex.style.opacity="0";vortex.style.transform="scale(1.15) rotate(0deg)";ring(running?on:off,running?onGlow:offGlow,0,-Math.PI/2);}
      function animate(next){next=!!next;if(frame)cancelAnimationFrame(frame);targetRunning=next;const from=running?on:off,to=next?on:off,fromGlow=running?onGlow:offGlow,toGlow=next?onGlow:offGlow,duration=Math.max(300,Number(next?p.startDuration:p.stopDuration)||2000),spins=next?3.6:3,t0=performance.now();function step(now){const raw=Math.min(1,(now-t0)/duration),e=ease(raw),angle=-Math.PI/2+Math.PI*2*spins*e,vortexPhase=Math.sin(Math.PI*raw),vortexScale=1.15-.94*e;ring(mix(from,to,e),mix(fromGlow,toGlow,e),raw,angle);vortex.style.opacity=String(vortexPhase*vortexIntensity);vortex.style.transform="scale("+vortexScale+") rotate("+(spins*720*e)+"deg)";vortex.style.filter="contrast(1.4) brightness("+(.68+vortexPhase*.55)+") blur("+(1-vortexPhase)*1.2+"px)";if(raw>=.48)button.classList.toggle("on",next);if(raw<1)frame=requestAnimationFrame(step);else{frame=0;stable(next);}}frame=requestAnimationFrame(step);}
