(function (runtime) {
  ...
  runtime.register({
    ...
    template: '<div class="sss-root"><button class="sss-button" type="button" aria-label="System start stop"><canvas width="300" height="300"></canvas><span class="sss-face"><span class="sss-pill"></span><span class="sss-system">SYSTEM</span><span class="sss-label sss-start">START</span><span class="sss-label sss-stop">STOP</span></span></button></div>',
    ...
    mount(root, context) {
      const button = root.querySelector(".sss-button"), canvas = root.querySelector("canvas"), ctx = canvas.getContext("2d"), pill = root.querySelector(".sss-pill"), system = root.querySelector(".sss-system"), startLabel = root.querySelector(".sss-start"), stopLabel = root.querySelector(".sss-stop"), p = context.options.properties || {};
      ...
      const off = p.offColor || "#dc2626", offGlow = p.offGlowColor || "#fca5a5", on = p.onColor || "#22c55e", onGlow = p.onGlowColor || "#86efac", intensity = Math.max(0, Math.min(1, Number(p.swirlIntensity || 72) / 100));
      ...
      function stable(value){running=targetRunning=!!value;button.classList.toggle("on",running);button.setAttribute("aria-pressed",running?"true":"false");ring(running?on:off,running?onGlow:offGlow,0,-Math.PI/2);}
      function animate(next){next=!!next;if(frame)cancelAnimationFrame(frame);targetRunning=next;const from=running?on:off,to=next?on:off,fromGlow=running?onGlow:offGlow,toGlow=next?onGlow:offGlow,duration=Math.max(300,Number(next?p.startDuration:p.stopDuration)||2000),spins=next?3.6:3,t0=performance.now();function step(now){const raw=Math.min(1,(now-t0)/duration),e=ease(raw),angle=-Math.PI/2+Math.PI*2*spins*e;ring(mix(from,to,e),mix(fromGlow,toGlow,e),raw,angle);if(raw>=.48)button.classList.toggle("on",next);if(raw<1)frame=requestAnimationFrame(step);else{frame=0;stable(next);}}frame=requestAnimationFrame(step);}
