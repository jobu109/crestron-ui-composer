...
run("Help includes a complete built-in general user manual", () => {
  ...
  ["JavaScript-generated parts", "Original versus Composer troubleshooting", "Migrating components made with the earlier Workbench"].forEach((heading) => assert.ok(markup.includes(heading)));
...
run("Component Creator provides one unified import and starter workflow", () => {
  ...
  assert.ok(html.includes("Composer properties</span>"));
  assert.ok(html.includes("Parts</span>"));
...
run("custom element picker classifies elements and generates standard capabilities", () => {
  ...
  assert.ok(editor.includes('upsertCustomWorkbenchMapping("property", mapping, true)'));
  assert.ok(editor.includes('upsertCustomWorkbenchMapping("connection", mapping, true)'));
  assert.ok(editor.includes("registerCustomWorkbenchPropertyCapability(propertyDefinition, targetSelector, resolved, false)"));
  assert.ok(editor.includes("registerCustomWorkbenchConnectionCapability(signalDefinition, targetSelector, resolved, false)"));
  assert.ok(html.includes('id="custom-component-original-preview"'));
  assert.ok(html.includes("Original authored component"));
  assert.ok(html.includes("Composer component"));
  assert.ok(css.includes(".custom-preview-comparison"));
  assert.ok(editor.includes("customOriginalSourceSnapshot || currentCustomSourceSnapshot()"));
  assert.ok(html.includes('id="custom-simulator-properties"'));
  assert.ok(editor.includes("customSimulatorPropertyValues = new Map()"));
  assert.ok(editor.includes("customSimulatorSignalValues = new Map()"));
  assert.ok(editor.includes("customSimulatorPropertyValues.has(property.key)"));
  ...
  assert.ok(editor.includes("scopedPropertyKeys = new Map()"));
  assert.ok(editor.includes("customElementPropertyKey(definition.key, selector)"));
  assert.ok(editor.includes("scopedPropertyKeys.get(key) || key"));
  assert.ok(editor.includes("friendlyCustomPartName({ metadata: customPickedElement, role })"));
...
run("custom components include reversible compatibility auditing and safe repairs", () => {
  ...
  assert.ok(editor.includes('if (customWizardStep === 4) runCustomComponentSelfTestSafely();'));
...
run("custom component creation is gated by blocking automated errors", () => {
  ...
  assert.ok(!editor.includes("readiness.confidence"));
  assert.ok(html.includes("Blocking errors must be corrected before creation"));
  assert.ok(editor.includes("function customReadinessFindingAction(message)"));
  assert.ok(editor.includes("function customReadinessFindingHelp(message, blocking)"));
  assert.ok(editor.includes("Blocking errors ("));
  assert.ok(editor.includes("Review notes ("));
  assert.ok(editor.includes("Open exact connection"));
  assert.ok(editor.includes("Select component part"));
  assert.ok(editor.includes("Open source line"));
  assert.ok(css.includes(".custom-readiness-group.blocking"));
...
run("component creation uses a source-first part-first five-step workflow", () => {
  const editor = read("editor.js"), html = read("editor.html"), css = read("editor.css");
  assert.equal((html.match(/data-custom-wizard-step=/g) || []).length, 5);
  assert.ok(html.includes("Source &amp; preview</span>"));
  assert.ok(html.includes("Parts</span>"));
  assert.ok(html.includes("Composer properties</span>"));
  assert.ok(html.includes("Crestron connections</span>"));
  ...
  assert.ok(editor.includes("if (customWizardStep === 4) refreshCustomPreview()"));
