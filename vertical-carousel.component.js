  ...
  global.ComposerRuntime.register({
    ...
    signals: [
      {
        ...
        type: "analog",
        direction: "output",
        defaultValue: "VerticalCarousel.Selected.Set",
      },
      {
        key: "feedback",
        ...
        type: "analog",
        direction: "input",
        defaultValue: "VerticalCarousel.Selected.Value",
      },
      {
        key: "count",
        name: "Number of slides (overrides default)",
        type: "analog",
        direction: "input",
        defaultValue: "VerticalCarousel.SlideCount",
      },
    ],
    signalGroups: [
    ...
    properties: [
      ...
      },
      {
        ...
        itemName: "Slide",
        defaultValue: "Source 1|Source 2|Source 3|Source 4|Source 5",
      },
      {
        key: "pressBase",
        name: "Press base / pattern",
        type: "text",
        defaultValue: "VerticalCarousel.Slides.{n}.Press",
        signalSetting: true,
      },
      {
        key: "pressIncrement",
        name: "Press increment",
        type: "number",
        defaultValue: 1,
        signalSetting: true,
      },
      {
        ...
        type: "text",
        defaultValue: "VerticalCarousel.Slides.{n}.Selected",
        signalSetting: true,
      },
      {
      ...
      },
      {
        ...
        type: "text",
        defaultValue: "VerticalCarousel.Slides.{n}.Label",
        signalSetting: true,
      },
      {
        key: "labelIncrement",
    ...
    mount(root, context) {
      ...
      const glow = Number(p.activeGlowOpacity) ?? 80;
      root.style.setProperty("--active-label", p.activeLabelColor || "#04aa8e");
      root.style.setProperty(
        "--inactive-label",
        p.inactiveLabelColor || "#fff",
      );
      root.style.setProperty(
        "--slide-color",
      ...
      root.style.setProperty(
        "--active-glow-light",
        rgba(p.activeGlowColor || "#04aa8e", glow / 2),
      );
      root.style.setProperty("--arrow-color", p.arrowColor || "#808080");
      root.style.setProperty(
        "--arrow-bg",
      ...
      function select(index, publish) {
        ...
        root.dispatchEvent(
          new CustomEvent("composer-scroll-position", {
            detail: { axis: "vertical", value: current },
          }),
        );
        if (publish) {
          context.signals.publish("set", current);
          const signal = address(p.pressBase, p.pressIncrement, current);
      ...
      function render(count) {
        count = Math.max(1, Math.min(20, Math.round(Number(count) || 5)));
        current = Math.min(current, count - 1);
        track.innerHTML = "";
        for (let i = 0; i < count; i++) {
          const slide = document.createElement("div"),
            span = document.createElement("span");
          slide.className = "glass-slide" + (i === current ? " active" : "");
          span.className = "glass-label";
          span.textContent = labels[i] ?? "";
          slide.appendChild(span);
          slide.onclick = () => select(i, true);
      ...
      root.addEventListener("composer-scroll-return", (event) => {
        if (event.detail?.axis === "vertical") select(0, true);
      });
      track.onpointerdown = (e) => (startY = e.clientY);
      track.onpointerup = (e) => {
      ...
      context.signals.subscribe("count", (value) => {
        ...
        if (count > 0) render(count);
      });
      for (let i = 0; i < 20; i++) {
        context.signals.subscribeAddress(
          "digital",
          address(p.feedbackBase, p.feedbackIncrement, i),
          (value) => {
        ...
        context.signals.subscribeAddress(
          "serial",
          address(p.labelBase, p.labelIncrement, i),
          (value) => {
